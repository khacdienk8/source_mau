<?php  if(!defined('_source')) die("Error");
    ...
    if($id!='')
	{
		...
		//Chi tiết tin tức
		$d->reset();
		$sql_detail = "select id,ten$lang as ten,mota$lang as mota,noidung$lang as noidung,id_danhmuc,id_list,id_cat,title,keywords,description,ngaytao,luotxem,photo,type FROM #_news where hienthi=1 and tenkhongdau='$id' limit 0,1";
		$d->query($sql_detail);
		...
		$d->reset();
		$sql_hinhthem = "select id,ten$lang as ten,thumb,photo FROM #_hinhanh where id_hinhanh='".$row_detail['id']."' and type='".$row_detail['type']."' and hienthi=1 order by stt,id desc";
	...
	{
		$d->reset();
		$sql = "select id,ten$lang as ten,title,keywords,description,type FROM #_news_list where tenkhongdau='$id_list' limit 0,1";
		$d->query($sql);
		$title_bar = $d->fetch_array();
		if(empty($title_bar)){redirect("http://".$config_url.'/404.php');}
	...
	$d->reset();
	$sql = "select id,ten$lang as ten,tenkhongdau,thumb,photo,mota$lang as mota,ngaytao,file_upload, noidung$lang as noidung FROM #_news where $where limit $bg,$pageSize";		
	$d->query($sql);
	$tintuc = $d->result_array();	
	$url_link = getCurrentPageURL();

	if($id=='')
		$id = $tintuc[0]['tenkhongdau'];
