...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			...
			$data['photo'] = $photo;	
			//dump($photo);
			$data['thumb'] = create_thumb($data['photo'], 300 , 330 , _upload_sanpham,$file_name,1);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		if($d->update($data))
		{
			...
			if(trim($_POST['tag'])!=''){
			  ...
			  foreach ($arrTags as $tag)
			  {
				 ...
				 {
					  mysql_query("insert into table_tags(ten,type,id_pro) values ('$tag','$type','$id')");
			...
			for($i=0;$i<count($_FILES['files']['name']);$i++){
				if($_FILES['files']['name'][$i]!=''){
					if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
					{
						...
						$data1['photo'] = $photo;
						$data1['thumb'] = create_thumb($data1['photo'], 300 , 330 , _upload_hinhthem,$file_name,1);	
			...
			mysql_query("DELETE FROM table_product_price where id_pro = '$id'");
			for($i=0;$i<$count_size;$i++){
				if($_POST['sizesp_name'.$i])
				{
			
					$data2['size'] = $_POST['sizesp_name'.$i];
					$data2['price'] = $_POST['sizesp_price'.$i];
					$data2['id_pro'] = $id;
...
function delete_item(){
	...
	if(isset($_GET['id']))
	{
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_tags where id_pro='".$id."'";
			$d->query($sql);
...
function save_list(){
	...
	if($id){		
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 40, 40, _upload_sanpham,$file_name,2);		
...
function save_danhmuc(){
	...
	if($id){		
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 250, 250, _upload_sanpham,$file_name,1);
