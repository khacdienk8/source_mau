...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 285, 230, _upload_tintuc,$file_name,1);									
			$d->setTable('news');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		if($file_upload = upload_image("file_upload", _format_duoitailieu, _upload_tintuc,"file-".$file_name)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_tintuc.$row['file_upload']);									
			}
		}
	             ...
	             for($i=0;$i<count($_FILES['files']['name']);$i++){
	            	if($_FILES['files']['name'][$i]!=''){
						if(!in_array(($_FILES['files']['name'][$i]),$arr_file_del,true))
						{
							...
							$data1['photo'] = $photo;
							$data1['thumb'] = create_thumb($data1['photo'], 285, 230, _upload_hinhthem,$file_name,1);	
...
function save_loai(){
	...
	if($id){	
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name)){
			$data['photo'] = $photo;
			//$data['thumb'] = create_thumb($data['photo'], 200, 150, _upload_tintuc,$file_name,1);	
