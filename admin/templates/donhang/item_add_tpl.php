...
<form name="supplier" id="validate" class="form" action="index.php?com=order&act=save" method="post" enctype="multipart/form-data">
	<div class="widget">
        ...
        <div class="formRow">
			<label>Địa chỉ</label>
			<div class="formRight">
             <input type="text" name="diachi" title="Địa chỉ khách hàng" style="width:80% !important" id="diachi" class="tipS validate[required] read" value="<?=@$item['diachi']?>" />
			</div>
			<div class="clear"></div>
		</div>	
    ...
    <tbody>
        ...
        <tr id="productct<?=$chitietdonhang[$i]['id']?>">
          <td><?=$i+1?></td>
          <td>
          	<b><?=$chitietdonhang[$i]['ten']?></b><br>
          	<p>Size: <?=get_size_name($chitietdonhang[$i]['size'])?></p>
          	<p>Màu: <span style="width: 20px;height: 20px;display: inline-block;vertical-align: top;background:<?=$chitietdonhang[$i]['mausac']?>;"></p>
