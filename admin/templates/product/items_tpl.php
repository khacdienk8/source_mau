...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
      ...
      <tr>
        ...
        <td width="130px" class="none">Bình luận</td>
        <td class="tb_data_small">Khuyến mãi</td>
        <td class="tb_data_small">Bán Chạy</td>
        <td class="tb_data_small none">Sp mới</td>
        <td class="tb_data_small none">Theo yêu cầu</td>
        <td class="tb_data_small none">Hết hàng</td>
    ...
    <tbody>
        ...
        <td align="center">
            <a href="index.php?com=product&act=edit&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&type=<?=$_REQUEST['type']?>&p=<?=$_REQUEST['p']?>&id=<?=$items[$i]['id']?>" class="tipS SC_bold"><img src="<?=_upload_sanpham.$items[$i]['thumb']?>" style="max-height:40px; max-width:70px;" /></a>
        </td>
        
         <td align="center" class="none">
            <a href="index.php?com=comment&act=man&product_id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?>&id=<?=$items[$i]['id']?>" class="tipS SC_bold">
             <?php
			 		$d->reset();
					$sql = "select count(id) as dem from table_comment where product_id='".$items[$i]['id']."' and type='".$_GET['type']."'";
					$d->query($sql);
					$dem_comment = $d->result_array();	
			  ?>
            (<?=$dem_comment[0]['dem']?>) B.luận</a>
        </td>
        ...
        <td align="center" class="none">
          <a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['hethang']?>" data-val3="hethang" class="diamondToggle <?=($items[$i]['hethang']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a>   
        </td>
        <td align="center" >
